...
class SI_LocalSearches extends Pepper
{
	...
	function onRecord()
	{
 		...
 		$path	= $this->prefs['localSearchPath'];
 		$query	= $this->prefs['localSearchQuery'];
		...
		$found	= 0;
 		...
 		if (preg_match("#(".str_replace('.', '\.', implode('|', $this->Mint->domains)).")(:\d+)?{$path}[^\?]*\?(([^&]*&)*{$query}=(?!cache:[^:]+:)([^&]*)&?)#i", html_entity_decode($referer), $matches))
		{
			$match	= $matches[count($matches) - 1];
 			$terms	= $this->escapeSQL(rawurldecode(htmlentities($match)));
 			$found	= 1;
 		}
 		// if resource matches
...
HERE;
	...
	{
		$hasCrush = isset($this->Mint->cfg['pepperLookUp']['SI_SecretCrush']);
		$html = '';
		...
		$query = "SELECT `local_search_terms`, `local_search_found`, `referer`, `resource`, `resource_title`, `dt`{$additional_columns}
