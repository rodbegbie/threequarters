...
class FreeComment(models.Model):
    ...
    def save(self):
        ...
        if not self.id:
            ...
            mail_body = 'On %s, %s (%s, %s) posted the following comment on "%s" (%s):\n\n%s' % (self.submit_date.strftime("%A, %B %d, %Y at %I:%M %p"), self.person_name, self.person_email, self.person_url, self.get_content_object().content_object.title, self.get_content_object().get_content_object.get_absolute_url(), self.comment)
